...
When you're developing code like this, you might want to avoid repeatedly downloading data. We can use an `if ( )` statement to run code only if something is true. In this case only if the `co2_file` does *not* already exist, i.e. hasn't already been downloaded to our `data` folder. The `!` means 'not'. We'll see more logical operators in section \@ref(logicalTests).
...
co2_file <- "data/CO2_emissions.xlsx" # save it here
if (!file.exists(co2_file)) download.file(co2_url, co2_file, mode = "wb")
...
We've already seen the `function(parameter)` way to call a function in the `cos(pi/3)` example. Now we have `something <- function(parameter, parameter)`. This is a peculiarity of R that you just need to get used to. Think of it as saying: create `something` in the environment (without saying what it is just yet), then fill it (`<-`) with the results of the `function()`. While you might occasionally have used a function in Excel, for example, in R basically everything you do is (define and then) call functions. If you don't have the `something <- ` bit, then R will print the result (to the console). This is what happens with the `download.file()` call, though it returns a silent nothing, so nothing is printed. Instead it saves the downloaded file to the specified place.

If the data you're using are frequently updated, then you probably want to remove the `if ()` part. Or if running the code bit-by-bit, you can just select the `download.file()` and run that, instead of selecting the whole line.

Notice that R doesn't mind if a function is split over several lines.

The data come with a description and a disclaimer. See the Excel file, now in your project `data` folder, for details.
...
One simple, but useful tool in the 'explore your data' toolbox is `summary`.

```{r co2summary}
summary(aviation_co2)
```

The `summary` function is fairly basic, but it gives a quick feel for what's in the data. It is often more helpful with numeric than character variables, but also useful for spotting if there are missing values `NA`. So we can see there are *no* missing values here.

In this dataset we have:

* YEAR: An integer, not a date, but `read_xlsx` reads this as a real number (ie potentially having decimals) since we didn't tell it otherwise.
* MONTH: An integer, giving the month in the year. Again, this has been assumed to be real rather than integer.
* STATE_NAME, STATE_CODE: A long name and the 2-letter 'country code' derived from the ICAO 4-letter communication address. This 2-letter code is often used in air traffic management data, but can be misleading. [In these data, usage of the 2-letter code is relatively clean, but check out 'GC'.]
...
* Drop some variables. Here, just `select` everything _except_ `STATE_CODE`. ('Not' is a very thin `!`, so easily overlooked.)^[In fact, this step is here just to illustrate the selection step. It's not needed, as we'll see in the data wrangling chapters, or you could test by removing it.]
* Drop some rows. There are few families and business unaffected by COVID-19. Aviation is of course no exception, and the data from 2020 are really unusual for this reason. To keep things simple here, we omit data after 2019, using a `filter` that says 'keep only the values before 2020'. The `group_by` isn't _essential_ for the filter, but I think it's probably faster as written here, since the grouping process has already found all of the year 2020 values.
* Summarise. `group_by` the relevant variables, then summarise within those groups. Here we use the `across` variant of summarise, which allows us quickly to apply a function `sum` to multiple variables. We tell summarise to drop the grouping information once it's finished.
* Then we save the data into the `data` subdirectory of the project.
...
annual_co2 <- aviation_co2 |>
  ...
  filter(YEAR < 2020) |>
  summarise(across(c(CO2_QTY_TONNES, TF), sum), .groups = "drop") 
...
There are three other important ways to explore the data. Firstly in the environment pane (top right, CTRL-8), where you can click the first line, with name on, to see a summary. You should see something like this.

![](images/CO2inEnvironment.png)

You get much the same thing in the console by typing `str(annual_co2)` where `str` is for 'structure' or, for that matter, almost the same thing from `glimpse(annual_co2)`. 
...
Checking which variables are in the dataset doesn't get you very far. For the next step in understanding the data you've downloaded, it's hard to beat a graph. So we end this preliminary chapter by creating a simple graph. The tidyverse way of doing this is to use `ggplot`.
...
There are too many countries to give each its own shape (we'll see shapes used more effectively in section \@ref(sortedBarExercises)), but we can easily add a line to join the points for each state. [Working from `geom_point`, how do you think you would add a line?]

We need both to add a line, which follows the pattern of `geom_point`, and group by state. That's done in the same way that we coloured by year, in the aesthetics. There are several ways to plot a line. The most obvious one, having seen `geom_point` previously, is `geom_line`. However, this joins the points in x-axis order. We want the points joined in data order, so that's `geom_path` instead.
...
Then there are two states, with flights in the 0.25-0.5 million flights range and that diverge from the main trend line, respectively higher and lower. The remaining, smaller states rather overlap in this graph. There's a big difference in CO~2~ emissions between short- and long-haul flights, so a first guess at why there are differences between states (or between years) would be because of differences in the mix of distances flown. But that's as far as we'll go here.
